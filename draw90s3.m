function draw90s3(x,y,phi,L,Linewidth,color_en)
% DRAW90S3
% draw90s3(x,y,phi,L,Linewidth)
% draw some 90 degree "diad" frames at x,y,phi with angle phi
...
for i = 1:length(x)
    hx = line([x(i) x(i)+L*cos(phi(i))],[y(i) y(i)+L*sin(phi(i))],[phi(i), phi(i)]);
    hy = line([x(i) x(i)-L*sin(phi(i))],[y(i) y(i)+L*cos(phi(i))],[phi(i), phi(i)]);
